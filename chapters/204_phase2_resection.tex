...
Once the navigation problem is conclusively solved, we arrive at the ultimate surgical challenge: Phase 2. This is where the highly specialized \textbf{SnareNavAgent} takes the stage. The localized environment here is drastically different from Phase 1. Instead of continuously driving through a long tubular tunnel, we directly spawn the agent right near a \textbf{pre-positioned polyp} situated inside a strictly constrained \textbf{``capture zone.''} Now, the agent must execute a highly complex \textbf{dual-control manipulation task}. It has to perform simultaneous, uncoupled actions---it must delicately \textbf{steer the endoscope camera tip} \textit{and} concurrently \textbf{manipulate the snare tool mechanism} with absolute surgical precision.

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{assets/architecture-phase2.png}
\caption{The neural network architecture of the Phase 2 extraction agent featuring a multi-branch policy head.}
\label{fig:architecture_phase2}
\end{figure}

Figure~\ref{fig:architecture_phase2} depicts the advanced architecture explicitly designed for the Phase 2 agent. While the core backbone remains functionally similar to Phase 1---utilizing a robust \textbf{CNN for spatial feature extraction} from the $128 \times 128$ RGB input and an \textbf{LSTM module for temporal memory}---the final output structure is significantly more complex. To handle the new requirements, the \textbf{Policy Head} is definitively split into \textbf{three simultaneous action branches} to control multiple surgical tools at once: \textbf{Branch 0} for continuous steering, \textbf{Branch 1} for \textbf{sliding the snare wire} (Extend/Retract), and \textbf{Branch 2} for \textbf{clamping the snare loop} (Open/Close). This vital \textbf{multi-branch architecture} is what mathematically enables the agent to navigate and operate the snare tool concurrently.
...
An important implementation detail is that the snare closing animation is not just a simple scale transform. When Branch 3 commands a ``Close Loop'' action, the \texttt{SnareSlideController} actually manipulates a \textbf{SkinnedMeshRenderer blend shape}. It smoothly slides the virtual wire loop into the catheter, clamped strictly to a \textbf{0.7 maximum weight} so it visually matches the physical snare's mechanical limitations. The \texttt{DecisionRequester} runs with a \textbf{period of 5}, meaning the agent makes one decision every 5 physics steps - giving each action time to produce visible consequences before the next decision.

\subsection{The Move-Then-Verify Collision Engine}
One of the trickiest engineering achievements here is how we prevent the snare from clipping through the colon wall or the polyp tissue. Standard physics engines struggle terribly with complex, concave, thin-wire collisions.
...
A notable challenge arises here: The virtual simulation cannot natively calculate collisions between two complex concave meshes (like a snare loop and a bumpy colon wall). So we built a \textbf{dual-collision system}:
\begin{itemize}
...
\begin{itemize}
\item \textbf{Small polyp sphere (goal):} Valid \textit{only} in Layer 3. Reward $= 5.0 + 10.0 \times \text{normalizedBlendWeight}$, where $\text{normalizedBlendWeight} = \text{clamp}(\text{blendWeight} / 0.7)$. This means the tighter the snare closes, the higher the terminal reward - up to $+15.0$. Episode terminates successfully.
...
Crucially, this phase introduces an \textbf{Intrinsic Curiosity Module (ICM)} to combat the extreme scarcity of the final capture reward. Because successfully looping the polyp stalk requires a highly specific sequence of geometric alignments, a purely random policy would almost never discover it. The \textbf{ICM} solves this by generating an \textbf{intrinsic reward signal} whenever the agent encounters novel visual states or unpredictable physics interactions. This artificially incentivizes the agent to actively \textbf{manipulate the snare} and explore the polyp's surface, effectively bootstrapping the learning process before any true surgical milestones are reached. We set the \textbf{curiosity strength to 0.02}, providing just enough intrinsic motivation to break the initial learning plateau without overpowering the core extrinsic surgical objectives. This delicate balance of \textbf{exploration versus exploitation} is paramount for mastering complex, multi-step manipulation tasks in sparse-reward environments.
...
\clearpage
\subsection{Training Results and Exploitative Behaviors}
...
A primary difficulty in this phase was the agent's tendency to exploit simulation loopholes. It frequently probed the environment for edge cases, trying to phase through colliders or exploit simulation loopholes in the reward logic to reach the goal without actually performing the intended snaring sequence. Because of this, we had to redesign the collision blocks, the game logic, and the rewards repeatedly. It required \textbf{34 iterative refinements} to correctly constrain the environment parameters.

During this intense training, our most crucial metric wasn't just the average score, but the \textbf{standard deviation}. For example, if the average score was 25 but the standard deviation was 20, it meant the agent was reaching the goal sometimes, but constantly failing while attempting alternative exploitative strategies. Only when the standard deviation dropped down to around \textbf{3} did we know the agent had ceased these exploitative behaviors and was reliably executing the proper capture maneuver in every single epoch, making it ready for deployment.
