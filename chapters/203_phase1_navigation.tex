...
Prior to the resection phase, the agent must thoroughly learn to navigate through an unpredictable, highly convoluted environment. Phase 1 introduces the \textbf{EndoscopeNavAgent}---a \textbf{vision-only Deep Reinforcement Learning (RL) agent} whose sole, critical mission is to \textbf{steer the endoscope tip} safely through the tortuous geometry of the colon without perforating the sensitive mucosal wall. To achieve this, the agent acts strictly as the ``eyes and hands'' on the directional steering dials. Meanwhile, a separate, \textbf{scripted procedural hand} systematically pushes the endoscope forward along its longitudinal axis. This deliberate \textbf{decoupling of steering from insertion} dramatically \textbf{reduces the mathematical exploration complexity}. By offloading the forward momentum, it allows the policy network to dedicate all its computational resources to the truly difficult challenge: instantly \textbf{reading the complex visual feed}, interpreting anatomical contours, and dynamically \textbf{reacting to sharp curves} in real time to ensure safe navigation.


\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{assets/architecture-phase1.png}
\caption{The neural network architecture of the Phase 1 navigation agent.}
\label{fig:architecture_phase1}
\end{figure}

The agent relies exclusively on a \textbf{vision-only pipeline}, meaning no external positional sensors or coordinates are used. A \textbf{$128 \times 128$ RGB image} is captured and directly processed by a robust \textbf{2-layer Convolutional Neural Network (CNN)}. This CNN is explicitly designed to \textbf{extract critical spatial features}---such as specular highlights, shadows, and haustral folds---which are then heavily compressed and sequentially passed through a dense \textbf{Body MLP}. Furthermore, to accurately understand directional motion and anatomical context over time, the network architecture utilizes a \textbf{Long Short-Term Memory (LSTM) module} equipped with a \textbf{sequence length of 64}. This temporal component is vital for preventing the agent from getting disoriented in repetitive-looking segments of the colon. Finally, the network's output is split into two distinct branches: a \textbf{Value Head} dedicated to estimating predicted returns, and a \textbf{Policy Head} that conclusively dictates the immediate \textbf{steering action} across \textbf{five discrete choices} (Up, Down, Left, Right, or None).
...
Instead of making the agent learn when and how hard to push forward, we aimed to replicate clinical operational patterns. A human endoscopist does not advance the scope at a constant, uninterrupted speed. They push in short, controlled bursts, pause to evaluate the visual field, and then resume insertion.
...
The agent is completely \textbf{vision-only}. It relies entirely on intrinsic visual data without external coordinate systems. It processes a small RGB camera feed and infers depth and curvature purely from the visual \textbf{optic flow} scrolling past the lens. 
...
\item \textbf{LSTM Memory:} A single-layer LSTM with \textbf{sequence length 64} and \textbf{memory size 128} (hidden size 64). This temporal integration is a critical component. The colon is a highly aliased environment - one mucosal fold looks nearly identical to another. Without temporal memory, the agent cannot determine if it is advancing, retreating, or stationary.
\end{enumerate}
...
\begin{itemize}
    ...
    \item $R_{goal} = +10.0$: The primary objective. Awarded instantly when the agent successfully reaches the \texttt{GoalTriggerLogger} target deep inside the colon segment.
...
\subsection{Safe Resets \& Fallbacks}
A significant challenge involved the agent occasionally spawning inside the colon mesh during episode resets. Given the narrow environment, overlapping with the wall geometry triggers immediate and severe physics collisions.
...
Analyzing the telemetry reveals two distinct learning phases. For the first two million steps, the policy remained highly volatile. The agent frequently triggered massive penalty values by slamming the endoscope tip into the mucosal walls, evidenced by sharp, erratic dips in the cumulative reward graph. However, right around the 2.5-million step threshold, a critical paradigm shift occurred. The \textbf{Proximal Policy Optimization (PPO)} algorithm finally mapped the correlation between localized specular highlights and impending structural collisions. From that point onward, the reward curve smoothed out dramatically, charting a steady upward climb as the agent traded aggressive forward momentum for cautious, calculated angulation. This definitive stabilization proved that the neural network had successfully internalized the geometry of the colon, marking the successful completion of Phase 1.
