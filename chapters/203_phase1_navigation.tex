...
\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{assets/model-phase1-setup.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{assets/model-phase1-agent-running.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{assets/model-phase1-different-angels.png}
...
Training an RL agent is never a straightforward path. Initially, we tried to drastically speed up the process by spawning \textbf{20 agents simultaneously} and running the simulation at $20\times$ speed. Unfortunately, Unity struggled to distribute the physics load, bottlenecking on a single CPU core. This caused major instability and forced us to rewrite significant chunks of the environment.

\subsection{Training Results}
Eventually, we pivoted to a single-agent training pipeline. This let us carefully observe the agent's behavior during each run and iteratively tweak hyperparameters and reward structures. It took \textbf{15 complete retrain attempts} - with each run lasting anywhere from 8 to 30 hours - before we finally discovered the golden configuration that produced a reliable, collision-free driving policy. For Phase 1, the agent ultimately achieved a \textbf{best score of 14.67}, taking \textbf{6.5 million steps} over a grueling \textbf{4 days} of training.
