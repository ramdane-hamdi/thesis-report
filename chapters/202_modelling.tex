...
Before an AI agent can learn a medical procedure, it needs a believable virtual world to practice in. For this deep reinforcement learning environment, our 3D models must strike a careful balance: detailed enough to provide robust visual features, yet computationally efficient to maintain high simulation speeds. We divide the environment into four primary components: the endoscope, snare, colon, and polyp.
...
The endoscope acts as the \textbf{``eyes and hands''} of our system. Clinical endoscopes are highly sophisticated instruments, packing an HD camera, illumination, and steering mechanics into a flexible tube just \textbf{9 to 13 millimetres} thick. While standard colonoscopes are up to 170\,cm long, simulating their full soft-body physics in Unity would severely degrade the simulation frame rate and make deep reinforcement learning impossible. 
...
In a physical clinical setting, this angulation is achieved via two \textbf{nested control knobs} manipulated by the physician's thumb. These dials pull on internal \textbf{Bowden cables} (steering wires) running the entire 170\,cm length of the insertion tube, culminating in the bending section. This mechanical linkage introduces significant \textbf{hysteresis, friction, and nonlinear responsiveness}, commonly referred to in clinical practice as \textbf{``whip'' or ``backlash.''} By abstracting this intricate mechanical reality into a \textbf{kinematic joint} steered via direct torques, we bypass the computationally intensive task of simulating cable friction. Consequently, our agent learns an \textbf{idealized, instantaneous mapping} from visual input to tip angulation. This definitely introduces a notable \textbf{\textit{sim-to-real} gap} because the simulated endoscope feels significantly more responsive than a real one. However, we did this on purpose to isolate the core visual-navigation problem from the chaotic physics of mechanical cables. Future work could introduce \textbf{synthetic delay or noise} into these control signals to eventually bridge this gap for physical deployment.
...
Simulating a 170\,cm flexible tube via Finite Element Method (FEM) would cripple frame rates. Instead, we \textbf{decouple forward translation from tip articulation}. The tip attaches to an \textbf{invisible kinematic pusher} gliding along a centerline spline at \textbf{2\,cm/s}. By forcing the forward momentum onto a set path, we eliminate the complex dynamics of insertion (such as loop formation or colonic stretching) from the learning objective. This radical simplification ensures training remains computationally viable while still providing a rigorous testbed. By abstracting soft-body physics, the agent focuses purely on \textbf{local visual steering}, maintaining \textbf{hundreds of frames per second} for Deep RL.
...
\paragraph{Texture, lighting, and specular highlights.}
...
Morphologically, the polyp is modeled as a pedunculated structure with a glossy, red-tinted PBR shader mimicking \textbf{hyperemia} (increased blood flow). Using this complex mesh for collision detection would be computationally wasteful and mathematically imprecise. Instead, we use a \textbf{dual-collider proxy system}.
